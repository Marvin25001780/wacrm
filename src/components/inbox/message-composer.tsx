...
export function MessageComposer({
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === "ai_not_configured") {
          toast.error(tcommon("ai_not_configured"));
        } else {
          toast.error(data.error ?? tcommon("could_not_draft"));
        }
        return;
      }
      const draftText = typeof data.draft === "string" ? data.draft.trim() : "";
      if (!draftText) {
        toast.error(tcommon("assistant_no_reply"));
        return;
      }
      setText(draftText);
      ...
      toast.error(tcommon("could_not_reach_ai"));
    } finally {
      setDrafting(false);
    }
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(tinbox("file_too_large", { size: (file.size / 1024 / 1024).toFixed(1), kind, limit: Math.round(max / 1024 / 1024) }));
        return;
      }
      setBusy(true);
      try {
        ...
        toast.error(err instanceof Error ? err.message : tinbox("upload_failed"));
      } finally {
        setBusy(false);
      }
    },
  ...
  const finalizeRecording = useCallback(
    async (bytes: Uint8Array) => {
      ...
      if (file.size > MEDIA_MAX_BYTES_BY_KIND.audio) {
        toast.error(tinbox("recording_too_long"));
        return;
      }
      setBusy(true);
      try {
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof AudioContext === "undefined") {
      toast.error(tinbox("voice_not_supported"));
      return;
    }
    try {
      ...
      {draft ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-border bg-muted px-4 py-2.5">
          ...
          <span className="flex-1 text-sm text-foreground">
            {tinbox("recording", { current: formatDuration(recordSeconds), max: formatDuration(MAX_RECORDING_SECONDS) })}
          </span>
          <button
            type="button"
        ...
        <div className="flex items-end gap-2">
          ...
          <GatedButton
            ...
            title={readOnly ? undefined : tinbox("send_template")}
          ...
          <GatedButton
            ...
            gateReason="send messages"
            disabled={drafting}
            title={readOnly ? undefined : tinbox("draft_with_ai")}
          ...
          <textarea
            ...
            placeholder={
              readOnly
                ? tcommon("read_only_cant_reply")
                : sessionExpired
                  ? tinbox("session_expired_placeholder")
                  : tinbox("type_message")
            }
            ...
            title={readOnly ? tcommon("read_only_cant_send") : undefined}
            className={cn(
      ...
      <div className="mt-2 flex items-end gap-2">
        {draft.kind !== "audio" && (
          <input
            ...
            placeholder={tinbox("add_caption")}
